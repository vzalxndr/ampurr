...
def _apply_on_boot():
    ...
    it reads the config and applies the limit. no user output.
    """
    try:
...
def install():
    """creates the systemd service and enables it."""
...
def find_supported_battery():
    ...
    charge limiting and returns the path to its directory.
    """
...
def set_charge_limit(limit_value, battery_path):
    """sets a new charge limit and saves it for persistence."""
    if os.geteuid() != 0:
...
def run_cli():
    """defines the argument parser and runs the user-facing CLI logic."""
